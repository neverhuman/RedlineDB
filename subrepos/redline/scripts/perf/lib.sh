...
#
# Sourced by: full.sh and any new perf scripts.
#
...
REDLINE_HUB_ROOT="${REDLINE_HUB_ROOT:-$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)}"
...
# Print a compact summary of a JSONL file without introducing a second
# evidence parser.
perf_summarize_jsonl() {
  local jsonl="$1"
  bash "$REDLINE_HUB_ROOT/tools/evidence-processor/run.sh" perf-summary "$jsonl"
}
