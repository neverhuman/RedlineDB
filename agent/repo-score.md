...
- Decision: `advisory`
- Minimum score: `85`
- Caps applied: `fallback-soup-in-product-code, future-hostile-dead-language-in-product-code, agent-tool-supply-chain-gap, rust-bad-behavior, repo-rot-bad-behavior`
...
- Status: `review` hard=`0` warning=`57` files=`285`
...
| `ExactUnitDifferentName` | `Warning` | `rust` | 1 | 6 | `crates/redlinedb-sqlx/src/bridge.rs:179-180, crates/redlinedb-sqlx/src/bridge.rs:258-259, crates/redlinedb-sqlx/src/bridge.rs:263-264, crates/redlinedb-sqlx/src/bridge.rs:287-288, crates/redlinedb-sqlx/src/bridge.rs:302-303, crates/redlinedb-sqlx/src/bridge.rs:317-318, crates/redlinedb-sqlx/src/bridge.rs:399-400` | `same body appears under different names across files` |
...
| `ExactUnitDifferentName` | `Warning` | `rust` | 1 | 4 | `crates/redlinedb-sqlx/src/bridge.rs:165-166, crates/redlinedb-sqlx/src/bridge.rs:172-173, crates/redlinedb-sqlx/src/bridge.rs:244-245, crates/redlinedb-sqlx/src/bridge.rs:251-252` | `same body appears under different names across files` |
...
| `ExactUnitDifferentName` | `Warning` | `rust` | 1 | 4 | `crates/redlinedb-sqlx/src/bridge.rs:205-206, crates/redlinedb-sqlx/src/bridge.rs:211-212, crates/redlinedb-sqlx/src/bridge.rs:335-336` | `same body appears under different names across files` |
...
| `ExactUnitDifferentName` | `Warning` | `rust` | 2 | 5 | `crates/kernel/src/json/path_bytecode.rs:138-140, crates/sql/src/parser.rs:187-189` | `same body appears under different names across files` |
...
| `ExactUnitSameName` | `Warning` | `rust` | 1 | 6 | `crates/redlinedb-sqlx/src/bridge.rs:179-180, crates/redlinedb-sqlx/src/bridge.rs:258-259` | `same-name semantic unit copied across multiple files` |
...
| `ExactUnitSameName` | `Warning` | `rust` | 1 | 4 | `crates/redlinedb-sqlx/src/bridge.rs:211-212, crates/redlinedb-sqlx/src/bridge.rs:335-336` | `same-name semantic unit copied across multiple files` |
...
| Code shape and semantic surface | 12 | 8 | 0.96 | largest authored code file: crates/redlinedb-sqlx/src/bridge.rs (697 LOC); code file exceeds 500 LOC |
...
1. `medium` `shape` `.`
   ...
   Rerun: `just fast`
   Fingerprint: `sha256:013449dd64d635e2e115c092df7d9d6ad33179714f395aff9742f6fa9c7afa48`
   Evidence: largest authored code file: crates/redlinedb-sqlx/src/bridge.rs (697 LOC), code file exceeds 500 LOC, most code files stay under 300 LOC, copy-code advisory classes found: 57 (advisory only, no score impact)
...
4. `medium` `proof` `agent/repo-score.json:3608`
   ...
   Rerun: `just score`
   Fingerprint: `sha256:e5547bd0fd4daeb8313d4594c336b899f07c85b40b785597b598e544a612ab0b`
   Evidence: "// rejected instead of returning a fabricated `0` row."
5. `high` `vibe` `crates/cli/src/dot/schema.rs:135`
   Rule: `HLT-001-DEAD-MARKER`
...
6. `medium` `context` `crates/ffi/src/sqlite3_api/backup.rs:1`
   ...
   Rerun: `just score`
   Fingerprint: `sha256:f7ee3103eaa8ba468ffede7e937314005bf9d5a1567826b8efac3502b8a028d2`
   Evidence: detector=repo-rot.path.fake-versioned-source, path=crates/ffi/src/sqlite3_api/backup.rs, line=1, proof_window=None, snippet=//! SQLite-compatible online-backup symbol wrappers.
...
8. `high` `vibe` `crates/redlinedb-sqlx/src/lib.rs:13`
   Rule: `HLT-001-DEAD-MARKER`
   Check: `HLT-001-DEAD-MARKER:vibe` `hard` confidence `0.88`
   Route: TLR `Entropy`, lane `fast`, owner `public-rust-facade`
   Docs: `docs/audit-rubric.md#future-hostile-language-rule`
   ...
   Fix: remove or rename the marker, implement the intended behavior, model a typed unsupported state, or move docs/generated/vendor/product-copy text into an allowlisted context
   Rerun: `just fast`
   Fingerprint: `sha256:ba617adbf3475f56650f051c0ecae66b34c67be5a5530d8d52f7dc6c5ebce5bc`
   Evidence: crates/redlinedb-sqlx/src/lib.rs:13, future-hostile/dead-language term `dummy` appears
...
2. `medium` `HLT-027-HUMAN-REVIEW-EVIDENCE-GAP` `agent/repo-score.json` - attach raw CI logs, review receipts, and replayable commands instead of accepting claims or summaries
   Route: `Repair`/`audit`
3. `medium` `HLT-040-REPO-ROT-BAD-BEHAVIOR` `crates/ffi/src/sqlite3_api/backup.rs` - delete the stale copy, move history to VCS/archive tooling, or document owner, proof lane, expiry, and migration plan
   Route: `Context/setup`/`audit`
4. `high` `HLT-024-AGENT-TOOL-SUPPLY-GAP` `.github/workflows/jankurai.yml` - pin and review agent tools, MCP servers, hooks, and rule files; keep untrusted tool output separate from trusted policy
   Route: `Security, secrets, agency`/`security`
5. `high` `HLT-001-DEAD-MARKER` `crates/cli/src/dot/schema.rs` - collapse fallback chains into explicit typed states with bounded retry policy, telemetry, and documented repair guidance
   Route: `Entropy`/`fast`
6. `high` `HLT-001-DEAD-MARKER` `crates/redlinedb-sqlx/src/bridge.rs` - remove or rename the marker, implement the intended behavior, model a typed unsupported state, or move docs/generated/vendor/product-copy text into an allowlisted context
   Route: `Entropy`/`fast`
7. `high` `HLT-001-DEAD-MARKER` `crates/redlinedb-sqlx/src/lib.rs` - remove or rename the marker, implement the intended behavior, model a typed unsupported state, or move docs/generated/vendor/product-copy text into an allowlisted context
   Route: `Entropy`/`fast`
8. `high` `HLT-029-RUST-BAD-BEHAVIOR` `crates/sql/src/connection/database.rs` - replace the mutable static with explicit synchronization or scoped ownership
