...
fn parse_repo_inventory(
    ...
    for row in rows {
        ...
        if row.get("visibility").and_then(JsonValue::as_str) != Some("private") {
            return Err("Jeryu repository inventory row is not private".into());
        }
...
"#
    ...
    fn repository_api_identity_is_exact_and_unambiguous() {
        ...
        let response = json!({"repositories": [row.clone(), unrelated.clone()]});
        validate_repo_absent(&response, "veox/jain-fabric").unwrap();
