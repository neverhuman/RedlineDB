...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match args.next().as_deref() {
        ...
        Some("distributed-validate") => distributed_release::validate_command(args.collect())?,
        Some("release-dag") => distributed_release::dag_command(args.collect())?,
        Some("distributed-evidence-index") => {
            distributed_release::evidence_index_command(args.collect())?
        }
        ...
        _ => return Err("usage: splitctl refresh-ci-contract [--repo NAME]... | materialize [--repo NAME]... | host-ci-snapshot-request --source PATH --destination PATH --expected-uid UID --expected-gid GID --max-bytes BYTES | cargo-cache-stage --lock PATH [--lock PATH]... --source PATH --destination PATH --receipt PATH --expected-source-uid UID --expected-source-gid GID | manifest [--manifest PATH] [--json] | managed-repos [--manifest PATH] --json | host-ci-authority [--manifest PATH] --repo NAME | release-cargo-commands [--manifest PATH] --repo NAME | sync-derived-manifests [--manifest PATH] [--receipt PATH] [--apply] | jankurai-evidence --repository NAME --commit SHA --worktree PATH --report-root PATH --report PATH --auditor PATH --attempt-id ID --lane-conclusion success|failure [--lane-failure-reason REASON] --clean-tracked-tree-start BOOL --receipt PATH | validate-manifest [--manifest PATH] [--check-paths] [--check-derived] | validate-local-jeryu [--manifest PATH] [--skip-remotes] | validate-family [--manifest PATH] [--json PATH] | validate-family-lock [--manifest PATH] [--lock PATH] | regenerate-lock [--manifest PATH] [--output PATH] --apply | release-preflight [--manifest PATH] [--json PATH] | release-snapshot [--manifest PATH] [--json PATH] | release-status [--manifest PATH] [--json PATH] | distributed-validate --release-spec PATH --soak-status PATH --qualification PATH --release-dag PATH --caddy-routes-before PATH --caddy-routes-after PATH --caddy-proof PATH --redline-family-ci PATH --redline-lock PATH --redline-lock-mirror PATH --redline-jain-consumer PATH --redline-jeryu-consumer PATH --rollback-proof PATH --signature-receipt PATH --signature-receipt PATH | release-dag --manifest PATH --locked-cargo-graph PATH --release 9.0.0-distributed.1 [--json PATH --apply] | distributed-evidence-index --root PATH --release 9.0.0-distributed.1 [--historical-root PATH]... [--historical-root-if-present PATH]... [--absent-historical-root PATH]... [--json PATH --apply] | bootstrap-main --repo PATH --remote URL --reviewed-commit SHA [--receipt PATH] [--apply] | immutable-tag --repo PATH --remote URL --tag TAG --commit SHA --token-file PATH [--receipt PATH] [--apply] | verify-worktrees [--manifest PATH] [--receipt PATH] | preflight [--manifest PATH] [--json PATH] | source-coverage [--manifest PATH] [--json] | seal-source-inventory [--manifest PATH] --source-root PATH [--apply] | python-boundary | jeryu-doctor [--manifest PATH] | reconcile [--manifest PATH] [--base-ref REF] [--apply] [--json PATH] | bump-version [--manifest PATH] --from VERSION --new VERSION --rewrite-split-tags".into()),
    }
    Ok(())
}
