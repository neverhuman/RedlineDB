...
fn derive_key_bytes(
    ...
    for key in &index.keys {
        let attnum = match &key.source {
            ...
            // Phase-11 SQL-D A6: expression-source keys cannot be derived
            // by the kernel-only equivalence check (it lacks the SQL
            // evaluator); surface that the kernel-side check is not
            // meaningful for this row by returning an empty key. The SQL
            // exec layer maintains the index keys directly on DML so the
            // physical index stays correct; the equivalence pass only
            // catches kernel-internal divergence.
            IndexKeySource::Expression { .. } => return Ok(Vec::new()),
        };
        let value = record
