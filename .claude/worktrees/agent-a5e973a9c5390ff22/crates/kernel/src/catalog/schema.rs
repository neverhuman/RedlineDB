...
pub struct ColumnDef {
    ...
    /// Phase-11 SQL-D A6: GENERATED ALWAYS AS (expr) column. `None` for
    /// ordinary columns. The expression is stored as the verbatim SQL
    /// fragment so the SQL exec layer can re-parse and evaluate it.
    /// `kind` selects between STORED (computed at write time, persisted
    /// in the heap row) and VIRTUAL (computed at every read).
...
pub struct IndexDef {
    ...
    /// Phase-11 SQL-D A6: partial-index WHERE predicate, stored as
    /// verbatim SQL text. `None` for full indexes. The SQL exec layer
    /// re-parses and evaluates this predicate per row before maintaining
    /// the index, and the planner only uses a partial index when the
    /// query's WHERE predicate is provably implied by it (today: text
