...
pub enum IndexKeySource {
    ...
    /// Phase-11 SQL-D A6: expression index key. The expression is stored
    /// as the verbatim SQL text from the `CREATE INDEX` statement so the
    /// SQL exec layer can re-parse it through `sqlparser` and evaluate it
    /// against the row's values. `referenced_cols` enumerates the column
    /// ordinals the expression mentions so the SQL index-update path can
    /// quickly detect whether an `UPDATE` touched any input column and
    /// thus needs to re-emit the index key.
    Expression {
        sql: Box<str>,
