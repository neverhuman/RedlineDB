...
use super::schema::{
    CatalogMeta, CheckDef, ColumnDef, ConstraintDef, ConstraintKind, GeneratedColumnKind,
    GeneratedColumnSpec, IndexDef, NamespaceDef, SchemaEpoch, SchemaSnapshot, TableDef,
};
...
fn encode_column(out: &mut BytesWriter, column: &ColumnDef) -> Result<()> {
    ...
    // Format-version 3: optional GENERATED ALWAYS spec.
    write_opt_generated(out, column.generated.as_ref());
    Ok(())
}
...
fn decode_column(reader: &mut BytesReader<'_>, format_version: u64) -> Result<ColumnDef> {
    Ok(ColumnDef {
        ...
        generated: if format_version >= 3 {
            read_opt_generated(reader)?
        } else {
            None
        },
    })
...
fn encode_index(out: &mut BytesWriter, index: &IndexDef) -> Result<()> {
    ...
    // Format-version 3: partial-index predicate SQL fragment.
...
fn decode_index(reader: &mut BytesReader<'_>, format_version: u64) -> Result<IndexDef> {
    ...
    let predicate_sql = if format_version >= 3 {
...
fn encode_index_key(out: &mut BytesWriter, key: &IndexKeyDef) -> Result<()> {
    ...
    match &key.source {
        ...
        IndexKeySource::Expression {
            ...
            // Format-version 3: expression-source key.
            out.u8(1);
            write_str(out, sql);
...
fn decode_index_key(reader: &mut BytesReader<'_>, format_version: u64) -> Result<IndexKeyDef> {
    ...
    let source = match reader.u8()? {
        ...
        1 if format_version >= 3 => {
            let sql = read_box_str(reader)?;
            let count = reader.u32()? as usize;
            let mut referenced_cols = Vec::with_capacity(count);
            for _ in 0..count {
...
fn write_opt_generated(out: &mut BytesWriter, value: Option<&GeneratedColumnSpec>) {
    match value {
        Some(spec) => {
            out.bool(true);
...
fn read_opt_generated(reader: &mut BytesReader<'_>) -> Result<Option<GeneratedColumnSpec>> {
    if !reader.bool()? {
        return Ok(None);
    }
    let kind = match reader.u8()? {
        0 => GeneratedColumnKind::Stored,
        1 => GeneratedColumnKind::Virtual,
        _ => return Err(Error::CatalogCorrupt("invalid generated column kind")),
    };
    let expr_sql = read_box_str(reader)?;
    Ok(Some(GeneratedColumnSpec { kind, expr_sql }))
}
