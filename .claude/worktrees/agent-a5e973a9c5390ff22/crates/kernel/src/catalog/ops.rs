...
use super::ddl::{
    ...
    CreateTableSpec, DropIndexSpec, DropTableSpec, GeneratedColumnDef, IndexColumnSpec,
...
pub fn apply_create_table(
    ...
    for (ordinal, column) in spec.columns.iter().enumerate() {
        ...
        columns.push(ColumnDef {
            ...
            default_value,
            default_expr,
            generated: column.generated.as_ref().map(generated_spec_from_def),
        });
    }
...
fn build_index_keys(table: &TableDef, columns: &[IndexColumnSpec]) -> Result<Vec<IndexKeyDef>> {
    ...
    for (idx, column) in columns.iter().enumerate() {
        if let Some(expr_sql) = column.expr_sql.as_deref() {
            // Phase-11 SQL-D A6: expression-source key. The parser
            // already validated the SQL fragment; we just need a slot
            // that the executor's `build_index_key` recognises.
            keys.push(IndexKeyDef {
                ordinal: idx as u16,
                source: IndexKeySource::Expression {
                    sql: expr_sql.to_owned().into_boxed_str(),
...
fn generated_spec_from_def(def: &GeneratedColumnDef) -> super::schema::GeneratedColumnSpec {
    super::schema::GeneratedColumnSpec {
        kind: def.kind,
        expr_sql: def.expr_sql.clone().into_boxed_str(),
    }
}
