...
impl Engine {
    ...
    fn backfill_index(
        ...
        // Phase-11 SQL-D A6: expression-source / partial indexes are
        // backfilled by the SQL exec layer (it has the SQL evaluator).
        // The kernel-only backfill cannot compute the keys here, so we
        // skip; the SQL layer's `apply_create_index` companion is the
        // single source of truth for those index shapes.
        ...
        for (row_id, _ptr) in entries {
            ...
            for key in &index.keys {
                let attnum = match &key.source {
                    ...
                    IndexKeySource::Expression { .. } => unreachable!(
                        "expression keys are filtered before backfill"
                    ),
                };
                let value = record
