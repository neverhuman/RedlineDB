...
pub(crate) fn bind_create_index(
    ...
    for column in create_index.columns {
        match convert_index_column(column.clone()) {
            Ok(c) => columns.push(c),
            Err(_) => {
                // Phase-11 SQL-D A6: expression-source index column. The
                // verbatim SQL fragment is stashed on the IndexColumnSpec
                // so the executor can re-parse and evaluate it per row.
                let expr_sql = column.column.expr.to_string();
                let referenced = super::helpers::ddl::index_expr_referenced_cols(
                    &column.column.expr,
                );
    ...
    // Phase-11 SQL-D A6: thread the partial-index predicate through to
    // the kernel as its verbatim SQL fragment; the executor re-parses
    // it per DML so the predicate semantics match exactly what the
    // catalog text declares.
