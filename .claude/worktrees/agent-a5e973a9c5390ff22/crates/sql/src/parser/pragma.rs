...
fn pragma_column_rows(
    ...
    if let Some(ordinal) = table.rowid_alias_column {
        ...
        for (position, key) in index.keys.iter().enumerate() {
            if let redlinedb_kernel::catalog::IndexKeySource::Column { attnum } = &key.source
                && let Some(slot) = pk.get_mut(*attnum as usize)
            {
...
fn pragma_index_info_rows(
    ...
    for (seqno, key) in index.keys.iter().enumerate() {
        let (attnum, name) = match &key.source {
            redlinedb_kernel::catalog::IndexKeySource::Column { attnum } => {
                ...
                (*attnum as i64, column.name.as_ref().to_owned())
            }
            redlinedb_kernel::catalog::IndexKeySource::Expression { sql, .. } => {
                // SQLite reports expression-source keys as cid=-1 with
                // NULL name in `pragma_index_info`; we emit the SQL
                // fragment so tools can identify the expression.
                (-1, sql.as_ref().to_owned())
            }
        };
        rows.push(vec![
            SqlValue::Integer(seqno as i64),
            SqlValue::Integer(attnum),
