use redlinedb_kernel::catalog::{
    ColumnConstraintSpec, ColumnSpec, ConflictAction, DbName, ExprAst, GeneratedColumnDef,
    GeneratedColumnKind, IndexColumnSpec, OwnedValue, SortDir, TableConstraintSpec,
};
use sqlparser::ast::{
    ColumnDef, ColumnOption, Expr, GeneratedExpressionMode, IndexColumn, ObjectNamePart,
};
...
pub(crate) fn convert_column_def(
    ...
    let mut generated: Option<GeneratedColumnDef> = None;
    ...
    for option in column.options {
        match option.option {
            ...
            ColumnOption::Generated {
                ...
                // or SELECT (VIRTUAL) time. SQLite uses VIRTUAL when
                // neither STORED nor VIRTUAL is specified.
...
/// Phase-11 SQL-D A6: enumerate column identifier names mentioned
/// inside an expression-source index column. The catalog stores these
/// so the SQL UPDATE path can decide whether an UPDATE touched an input
/// to the expression and therefore requires re-emitting the key. We
/// don't resolve to ordinals here — the kernel side uses the names to
/// look up ordinals at evaluation time.
pub(crate) fn index_expr_referenced_cols(_expr: &Expr) -> Vec<u16> {
    // Resolution to ordinals happens lazily inside the SQL exec layer
    // (where the TableDef is in scope). Storing an empty vector here is
    // the conservative default: the SQL exec layer recomputes the
    // referenced set lazily and pessimistically rebuilds the key on
    // any UPDATE.
    Vec::new()
}
