...
pub(crate) fn load_table_row_by_rowid(
        ...
        && table_id == table.table_id.0
    {
        ...
        // read time. STORED columns were already persisted at write
        // time so we leave their slots alone.
...
fn materialize_virtual_generated_columns(
    ...
    let has_virtual = table
        .columns
        .iter()
        .any(|c| matches!(c.generated.as_ref().map(|g| g.kind), Some(GeneratedColumnKind::Virtual)));
    if !has_virtual {
        return Ok(values);
    }
