...
/// — see `select_top::project_with_virtual_generated` — and are left
...
pub(crate) fn compute_stored_generated_columns(
    ...
    // Snapshot the input values BEFORE we start overwriting generated
    // columns, so expressions like `c = a + b` always see the original
    // user-provided / defaulted inputs (independent of evaluation order).
