...
fn covering_projection_for_index(
    ...
    for (pos, key) in index.keys.iter().enumerate() {
        // Expression-source keys do not map back to a single source
        // column for covering reads — fall through to the heap path.
...
fn covering_order_satisfies(
    ...
    let redlinedb_kernel::catalog::IndexKeySource::Column { attnum } = first_key.source else {
        // Expression-source leading key: covering ORDER BY cannot rely
        // on cursor order matching the user's ORDER BY expression.
        return false;
    };
    table
        .columns
