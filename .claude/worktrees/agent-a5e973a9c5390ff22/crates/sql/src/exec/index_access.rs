...
pub(crate) fn try_match_index_access(
    ...
    for index in &table.indexes {
        ...
        // Phase-11 SQL-D A6: partial indexes are only safe to use when
        // the query's WHERE provably implies the partial-index WHERE.
        // The simplest provable case is byte-equal predicates after
        // whitespace/case normalisation — see `super::index_partial`.
        if index.predicate_sql.is_some()
            && !super::index_partial::query_implies_index_predicate(selection, index)
        {
            continue;
        }
        // Phase-11 SQL-D A6: expression-index leading-key match
        // (`WHERE lower(name) = 'foo'` against `CREATE INDEX ix ON
        // t(lower(name))`). The helper returns None for column-source
        // keys, so this falls through to the regular column path.
        if let Some(matched) =
            super::index_partial::try_match_expression_index(table, index, selection, bindings)
        {
            let key = encode_full_key(index, std::slice::from_ref(&matched.leading_value));
            return Some(IndexAccessMatch {
                index: Arc::new(index.clone()),
                kind: IndexProbeKind::PointLookup,
                probe: IndexProbe::Point { key },
                predicates: vec![matched.predicate_str],
                ordered_limit: None,
            });
        }
        let leading = match first_key.source {
            IndexKeySource::Column { attnum } => attnum as usize,
            IndexKeySource::Expression { .. } => continue,
        };
...
pub(crate) fn encode_full_key(index: &IndexDef, values: &[SqlValue]) -> Vec<u8> {
    ...
    for (key, value) in index.keys.iter().zip(values.iter()) {
        // Both column-source and expression-source keys land here; the
        // probe constructor (`try_match_index_access`,
        // `try_match_expression_index_access`) is responsible for
        // supplying the right pre-evaluated `value` per key.
        let _ = &key.source;
...
pub(crate) fn encode_prefix_key(index: &IndexDef, leading_values: &[SqlValue]) -> Vec<u8> {
