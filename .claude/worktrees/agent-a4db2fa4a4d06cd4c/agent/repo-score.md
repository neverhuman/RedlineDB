...
| Code shape and semantic surface | 12 | 4 | 0.48 | largest authored code file: crates/redlinedb/src/lib.rs (941 LOC); code file exceeds 500 LOC |
...
1. `medium` `shape` `.`
   ...
   Rerun: `just fast`
   Fingerprint: `sha256:a93d7f7eaa22f2f3dd5f873169e1a4cb6842df95df81dd762703e8727c562432`
   Evidence: largest authored code file: crates/redlinedb/src/lib.rs (941 LOC), code file exceeds 500 LOC, duplicate code block marker found, fallback soup marker found
...
12. `high` `vibe` `crates/bench/src/feature_workloads.rs:141`
   ...
   Rerun: `just fast`
   Fingerprint: `sha256:4bc2cfd8983e61af2ca7d0970f87d8440538fd3f1c99eb50d4a7553abcddb31f`
   Evidence: duplicate block also appears at crates/bench/src/chaos/helpers.rs:22
...
8. `high` `HLT-001-DEAD-MARKER` `crates/bench/src/bin/chaos_report/main.rs` - collapse fallback chains into explicit typed states with bounded retry policy, telemetry, and documented repair guidance
   Route: `Entropy`/`fast`
9. `high` `crates/bench/src/feature_workloads.rs` - extract the duplicated behavior behind one named boundary and add focused tests before changing behavior
   Route: `Entropy`/`fast`
