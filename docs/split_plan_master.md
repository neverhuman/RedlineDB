...
- **Fleet lanes** (host-only, siblings symlinked by `split-host-ci.sh` with `JAIN_NEEDS_SIBLINGS=1`; non-required but release-blocking evidence in Stage 8):
  ...
  - `coverage-report` — re-run `source_coverage.py` against the seed (and post-reconcile) manifest.
...
2. **The real gate: `<repo>/required` on the forge host runner** via `split-host-ci.sh <owner> <repo> <sha> <path> [check]` (forked from `~/jeryu-split/ops/ci/split-host-ci.sh`): verify forge `/api/v1/version` → governed worker count (`jeryu-ci-governor`) → `git worktree add --detach` (+ `git lfs pull` when the repo tracks LFS) → **symlink sibling repos next to the worktree** → hooks: `JAIN_NEEDS_ARTIFACTS=1` ⇒ `ln -s $SPLIT_ROOT/jain-starforge/artifacts $wt/artifacts`; `JAIN_NEEDS_SIBLINGS=1` ⇒ symlink all (fleet/integration lanes only) → persistent per-repo `CARGO_TARGET_DIR` + sccache, warmed cargo-git and vendor caches, scrubbed `JERYU_API_URL` for hermeticity → run `ops/ci/required.sh` (or named lane) → `POST $JERYU_BASE/repos/jeryu/<repo>/check-runs {"name":"<repo>/required", "head_sha":…, "conclusion":…}`.
