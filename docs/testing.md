...
| `sqlite-parity-scale-smoke`          | Local SQLite shell parity smoke over P0 memory cases from `crates/bench/sqlite_parity/`.  |
| `sqlite-parity-scale-ci`             | Local diagnostic wrapper for the pinned `redline-testing` SQLite parity gate; report output uses `--local-diagnostics`, not official evidence. |
...
rerunning the benchmark. `sqlite-parity-scale-ci` remains a local diagnostic and
uses unbound report mode only through `--local-diagnostics`. Reference-shell
capability skips are hard errors in the official lane; `approved-ci.txt` remains
only a local triage subset while older diagnostics still need it.
