...
- **Materializer solid.** `ops/split/materialize.py` has no stubs/TODOs; 6/6 self-tests pass. Implements LFS smudge + >1 MB assert, 3-patch apply w/ `git apply --check`, per-repo lockfile regen via bare-mirrors+insteadOf, bare-mirror emission, profile renderers. `reconcile.py` / `source_coverage.py` real and complete.
...
**Core principle:** the per-repo lanes/policies/docs are *generated from templates inside `materialize.py`* (security.sh is byte-identical across 19 → single template). **Fix the generator + templates, then re-materialize once.** Do NOT hand-edit the 19 repos.

- **Stage A — Freeze:** coordinate/pause the concurrent codex agents (there is no timer to mask); confirm HEADs/tags stable; treat current tags as immutable. Optionally make `materialize.py --init-git` idempotent (no commit/tag rewrite when the repo already matches source; guard `materialize.py:2540-2548,2663`) so accidental re-runs don't churn.
