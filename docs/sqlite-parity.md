...
| `PRAGMA journal_mode` (memory/off/delete) | partial | `crates/sql/tests/parity_pragma_tv.rs` | sql-parser-planner-executor | Round-trip on the supported subset; `wal`/`truncate`/`persist` rejected with a message naming the value. |
