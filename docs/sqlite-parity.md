...
| Compound `SELECT` (`UNION`, `INTERSECT`, `EXCEPT`) | pass | `crates/sql/tests/parity_compound_select.rs` | sql-parser-planner-executor | A2: UNION / UNION ALL / INTERSECT / EXCEPT all wired through the parser and exec layers; column-arity and type-class mismatches return diagnostic errors that match SQLite's error class. |
...
| Generated columns | pass | `crates/sql/tests/parity_generated_col.rs` | sql-parser-planner-executor | A6: `GENERATED ALWAYS AS (expr) [STORED|VIRTUAL]` parsed in `parser/helpers/ddl.rs`, stored on `ColumnDef.generated` (catalog v7). STORED computed at write time in `tail_build::compute_stored_generated_columns` and recomputed on UPDATE/UPSERT-DO-UPDATE; VIRTUAL materialised at read time in `tail_rows::materialize_virtual_generated_columns`. Explicit writes to generated columns rejected in `parser/dml.rs` with SQLite-style error string. Differential parity passing against rusqlite oracle (7/7 tests). |
| Partial indexes | pass | `crates/sql/tests/parity_partial_index.rs` | sql-parser-planner-executor | A6-indexes: `CREATE INDEX ... WHERE <predicate>` parses, persists the predicate SQL in the catalog, and the maintenance / index-DML layer only emits / removes entries when the predicate evaluates truthy for the candidate row. Planner advertises a partial index only when the query's top-level AND conjuncts contain the index predicate (modulo whitespace / case normalisation). |
| Expression indexes | pass | `crates/sql/tests/parity_expr_index.rs` | sql-parser-planner-executor | A6-indexes: `CREATE INDEX ix ON t(expr(col))` parses, persists the SQL key fragment, and the maintenance layer recomputes the key value on every INSERT / UPDATE / DELETE through the shared `eval_scalar` machinery. Planner advertises the index only for the matching `WHERE expr(col) = value` form; other shapes fall back to a scan. |
...
| ATTACH / DETACH | pass | `crates/sql/tests/parity_attach.rs` | sql-parser-planner-executor | A2: per-connection alias map (`crate::exec::attach::AttachMap`) opened via `ATTACH DATABASE 'file' AS alias` and torn down by `DETACH DATABASE alias`. Reserved aliases `main` / `temp` return a SQLite-style error class. Cross-database `alias.table` resolution is wired through the planner. |
