...
2. **Local rehearsal.** Supply an explicit next-unused candidate identity, for
   ...
   - creates an automatically removed `git clone --no-local` detached at the
     ...
     empty environment, fixed tool paths, an isolated Cargo home, offline
     governed vendor input, and an external target directory; ignored local
   ...
   - binds the exact commit, tree, deterministic source-archive digest,
     ...
     configuration, vendor inventory, and sanitized environment in
