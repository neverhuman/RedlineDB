...
- `crates/bench` keeps local smoke and diagnostic helpers only.
...
| `crates/bench/` | Local parity smoke and diagnostics; not official evidence |
...
- `just sqlite-parity-scale-ci` remains a local SQLite parity diagnostic; CI now uses `redline-testing-official` for the official gate.
- `scripts/ci-local.sh all` mirrors the broader local CI surface when you need it.

The repository keeps only local smoke/triage helpers under source control. The
sole official source for SQLite, memory, beyond-SQLite, latency, chart,
README, Jankurai, and score evidence is the pinned
`neverhuman/redline-testing` release artifact and its processed evidence
bundle; `approved-ci.txt` remains as a local triage subset for older diagnostics
only.
