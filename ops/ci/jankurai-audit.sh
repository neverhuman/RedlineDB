...
mkdir -p "$TARGET_DIR/security" "$TARGET_DIR/proofbind" "$TARGET_DIR/proofmark" "$TARGET_DIR/rust"
JANKURAI_VERIFY_LOG="$TARGET_DIR/governed-jankurai.log"
...
step_audit_advisory() {
    ...
    force_full_smart_scan
    local -a audit_cmd=(jankurai audit . --mode advisory --baseline .jankurai/baselines/main.repo-score.json --json "$TARGET_DIR/repo-score.advisory.json" --md "$TARGET_DIR/repo-score.advisory.md" --sarif "$TARGET_DIR/jankurai.sarif" --github-step-summary "$TARGET_DIR/summary.md" --repair-queue-jsonl "$TARGET_DIR/repair-queue.jsonl" --no-score-history --policy "$AUDIT_POLICY")
    "${audit_cmd[@]}"
}
...
step_fetch_baseline() {
    ...
    install -m 0644 .jankurai/baselines/main.repo-score.json "$TARGET_DIR/accepted-baseline.json"
...
step_audit_ratchet() {
    ...
    bash scripts/check_audit_policy_mirror.sh
    force_full_smart_scan
    local -a ratchet_cmd=(jankurai audit . --mode ratchet --baseline "$TARGET_DIR/accepted-baseline.json" --json "$TARGET_DIR/repo-score.json" --md "$TARGET_DIR/repo-score.md" --repair-queue-jsonl "$TARGET_DIR/repair-queue.jsonl" --no-score-history --policy "$AUDIT_POLICY")
    "${ratchet_cmd[@]}" || rc=$?
...
# ---- 10) Copy-code audit ---------------------------------------------------
step_copy_code() {
    jankurai copy-code . --json "$TARGET_DIR/copy-code.json" --md "$TARGET_DIR/copy-code.md"
}
...
step_ux_qa() {
    if [ ! -f packages/ux-qa/dist/cli.js ]; then
        printf '{"status":"not_applicable","reason":"packages/ux-qa/dist/cli.js missing; no rendered web surface in this repo"}\n' \
            > "$TARGET_DIR/ux-qa.json"
        return 0
    fi

    jankurai ux audit --config .jankurai/ux-qa.toml --out "$TARGET_DIR/ux-qa.json"
}

# ---- 12) Governed binary hostile probes ------------------------------------
step_governed_jankurai_probes() {
    bash ops/ci/governed-jankurai-test.sh \
        | tee "$TARGET_DIR/governed-jankurai-test.log"
}

main() {
    ci_require_clean_head
    ci_require_governed_jankurai_logged "$JANKURAI_VERIFY_LOG"
    step_governed_jankurai_probes
    ...
    ci_verify_jankurai_report \
        "$TARGET_DIR/repo-score.json" \
        "$TARGET_DIR/governed-evidence.json"
    ci_require_clean_head
}
