...
for tool in git jq npm sha256sum realpath; do
...
[[ -d "$RUSTSEC_DB" && ! -L "$RUSTSEC_DB" \
  ...
  && -z "$(find "$RUSTSEC_DB" -type l -print -quit)" ]] \
  || fail "pinned local RustSec database custody is invalid"
[[ "$(git -C "$RUSTSEC_DB" rev-parse HEAD)" == "$RUSTSEC_DB_COMMIT" \
  && -z "$(git -C "$RUSTSEC_DB" status --porcelain)" ]] \
  || fail "pinned local RustSec database identity is invalid"
...
"$CARGO_AUDIT_BIN" audit --no-fetch --db "$RUSTSEC_DB" --deny warnings --json \
