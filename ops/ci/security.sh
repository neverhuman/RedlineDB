...
if repo_has apps/web/package-lock.json && has npm; then
  log "security: npm audit (apps/web, offline advisory cache)"
  # --offline resolves advisories from the local npm cache instead of the live
  # registry, so this stays blocking inside the sealed no-network sandbox.
