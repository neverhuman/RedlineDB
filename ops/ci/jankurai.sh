...
' agent/repo-score.json >/dev/null

jq -r '"jankurai strict gate: score=\(.score // 0) minimum=\(.minimum_score // .decision.minimum_score // 85) blockers=\((.conformance_blockers // []) | length) caps=\((.caps_applied // []) | length)"' agent/repo-score.json
