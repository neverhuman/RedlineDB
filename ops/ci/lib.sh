...
ci_verify_jankurai_report() {
    ...
    local head tree short report_head dirty auditor passed hard caps report_sha256
    ...
    [ "$report_head" = "$short" ] || {
        printf 'Jankurai report head mismatch: expected %s, got %s\n' "$short" "${report_head:-missing}" >&2
        return 1
    }
    [ "$dirty" = "false" ] || {
