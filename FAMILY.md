...
## Develop across the family

The hub stays thin and the repos build independently, so there is no umbrella workspace.
To hack on the whole stack at once, `just fuse` clones the three siblings into a
gitignored `.fusion/` tree and generates a dev loop — see the **Develop the whole stack**
section of [`README.md`](README.md). How the family is evolving (the old `~/redlineDB`
checkout and `redline-testing` consolidation) is tracked in
