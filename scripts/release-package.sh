...
if [[ "${REDLINE_TESTING_RELEASE_INTERNAL:-0}" != 1 ]]; then
  ...
  forge_tag_receipt="$release_sandbox/tag-identity.json"
  ops/ci/release-tag-identity.sh "$tag" "$forge_tag_receipt" "$repo_root"
  forge_tag_receipt_sha="$(
    sha256sum "$forge_tag_receipt" | awk '{print $1}'
  )"
  ...
  path_remap_policy="release-path-remap/v1"
  cargo_encoded_rustflags="$(
    printf '%s\x1f%s\x1f%s\x1f%s\x1f%s' \
      "--remap-path-prefix=$projection=/source" \
      "--remap-path-prefix=$cargo_home=/cargo-home" \
      "--remap-path-prefix=$cargo_target=/cargo-target" \
      "--remap-path-prefix=$build_home=/home" \
      "--remap-path-prefix=$build_tmp=/tmp"
  )"
  environment_sha="$(
    printf '%s\n' \
      "CARGO_HOME=<isolated-cargo-home>" \
      "CARGO_ENCODED_RUSTFLAGS=$path_remap_policy" \
      ...
      "FORGE_TAG_IDENTITY=$forge_tag_receipt_sha" \
  ...
  env -i \
    ...
    REDLINE_TESTING_TAG_IDENTITY_RECEIPT="$forge_tag_receipt" \
    REDLINE_TESTING_TAG_IDENTITY_RECEIPT_SHA256="$forge_tag_receipt_sha" \
...
provided_tag_receipt="${REDLINE_TESTING_TAG_IDENTITY_RECEIPT:?}"
provided_tag_receipt_sha="${REDLINE_TESTING_TAG_IDENTITY_RECEIPT_SHA256:?}"
[[ -f "$provided_tag_receipt" && ! -L "$provided_tag_receipt" \
    && "$(sha256sum "$provided_tag_receipt" | awk '{print $1}')" \
        == "$provided_tag_receipt_sha" ]] || {
  printf 'release package: authenticated tag identity receipt is unsafe\n' >&2
  exit 1
}
cp "$provided_tag_receipt" "$tag_identity_receipt"
jq -e --arg tag "$tag" \
  --arg head "$(git rev-parse --verify HEAD)" \
  --arg tree "$(git rev-parse --verify 'HEAD^{tree}')" '
    .schema_version == "redline.testing.release-tag-identity/v2"
    and .release_tag == $tag
    and .release_commit == $head
    and .release_tree == $tree
    and (.tag_state == "planned" or .tag_state == "live")
    and (
      (.tag_state == "planned"
        and .local_tag_object == "" and .local_tag_commit == ""
        and .forge_tag_object == "" and .forge_tag_commit == "")
      or
      (.tag_state == "live"
        and .local_tag_object == .forge_tag_object
        and .local_tag_commit == $head and .forge_tag_commit == $head)
    )
  ' "$tag_identity_receipt" >/dev/null || {
    printf 'release package: authenticated tag identity receipt is invalid\n' >&2
    exit 1
}
