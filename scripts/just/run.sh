...
sqlite_parity_report_args() {
  ...
  if [ "$official_evidence" = "local-diagnostics" ]; then
    sqlite_parity_report_args_result+=(--local-diagnostics)
  else
...
run_redline_testing_sqlite_parity() {
  local sqlite_parity_root
  local redline_testing_bin
  local raw_tmp
  local updated_date
  sqlite_parity_root="benchmark-results/sqlite-parity/latest"
  raw_tmp="target/sqlite-parity/full-corpus-ci.raw.jsonl"
  updated_date="${REDLINEDB_SQLITE_PARITY_UPDATED_DATE:-$(date -u +%F)}"

  mkdir -p "$sqlite_parity_root" assets target/sqlite-parity
  rm -f "$raw_tmp"
  prepare_redline_testing_target "sqlite-parity"
  redline_testing_bin="$(ci_install_redline_testing)"
  load_redline_testing_provenance "$redline_testing_bin"
  "$redline_testing_bin" run \
    ...
    --suite sqlite_parity \
    --workers "$sqlite_parity_jobs" \
    --tmp-root "$(redline_testing_tmp_root)" \
    --repetitions "$sqlite_parity_repetitions" \
    --warmup "$sqlite_parity_warmup" \
    --output "$raw_tmp"
  mv "$raw_tmp" "$sqlite_parity_root/raw.jsonl"
  mv "$(dirname "$raw_tmp")/provenance.json" "$sqlite_parity_root/provenance.json"
  printf '%s\n' "$updated_date" > "$sqlite_parity_root/UPDATED_DATE"
  sqlite_parity_report_args "$updated_date" local-diagnostics
...
case "$lane" in
  ...
  sqlite-parity-volatile-sentinel)
    ...
    redline_testing_bin="$(ci_install_redline_testing)"
    load_redline_testing_provenance "$redline_testing_bin"
    "$redline_testing_bin" sentinel --input target/sqlite-parity/volatile-fastpath-sentinel.jsonl --ceiling-ns 00003=250000000 --ceiling-ns 00274=200000000 --ceiling-ns 00807=500000000 --ceiling-ns 00949=750000000 ${REDLINEDB_VOLATILE_SENTINEL_ENFORCE:+--enforce}
    ;;
