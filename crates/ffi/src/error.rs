...
pub extern "C" fn rldb_free(ptr: *mut c_void) {
    if !ptr.is_null() {
        // SAFETY: every pointer handed to rldb_free is minted by
        // errmsg_to_c_string in crates/ffi/src/util.rs and released exactly
        // once here through the matching constructor/destructor pair.
        crate::util::free_errmsg(ptr);
    }
}
