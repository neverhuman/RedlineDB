...
use super::cells::{LeafCell, LeafCellRef, LeafEntry, leaf_entry_visible};
...
/// chain as [`IndexCursor`] but decodes cells in place, avoiding the
/// per-leaf `Vec<LeafEntry>` materialisation that the general cursor
/// still uses for legacy compatibility.
    ...
    fn matches_ref_cached(&self, entry: &LeafCellRef<'_>, cache: &mut Vec<(TxId, bool)>) -> bool {
        match self.view.inner {
            ...
            SnapshotViewKind::Visible {
                ...
                if entry.delete_tx == super::NON_TRANSACTIONAL_DELETE_TX {
                    return false;
                }
