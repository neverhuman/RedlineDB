//! Pre-open dispatch for the lite shell.
//!
//! Adapted from `crates/cli/src/shellzero.rs`. Same audited surface: a
//! handful of dot-commands plus fromless scalar `SELECT`s. Anything we
//! can't statically evaluate returns `Unsupported` and the caller falls
//! back to exec'ing the full `redlinedb` binary.

/// Result of a single dot-command.
pub enum DotResult {
    Ok,
    Exit(i32),
    Unsupported,
...
/// Outcome of dispatching the joined statement buffer.
pub enum Dispatch {
    /// Everything handled. Output bytes ready to write, then exit with code.
    Handled { out: String, code: i32 },
    /// Some statement was outside the audited surface; fall through.
    Unsupported,
}
    ...
    for raw in split_statements(&joined) {
        ...
        if let Some(rest) = trimmed.strip_prefix('.') {
            match handle_dot(rest, &mut out) {
                ...
                DotResult::Unsupported => return Dispatch::Unsupported,
            }
        } else {
            let values = match eval_fromless_select(trimmed) {
                Some(v) => v,
                None => return Dispatch::Unsupported,
            };
            render_row(
                &values,
                args.separator,
...
fn handle_dot(rest: &str, out: &mut String) -> DotResult {
    ...
    let dot_args: Vec<&str> = parts.collect();
    match head {
        "help" => {
            print_help_into(out);
            DotResult::Ok
        ...
        "print" => {
            let joined = dot_args.join(" ");
            out.push_str(&joined);
            out.push('\n');
            DotResult::Ok
        }
        "exit" | "quit" => {
            let code = match dot_args.first() {
