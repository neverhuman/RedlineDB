...
#[inline]
fn redline_column_parts(column: &RedlineColumn) -> (usize, &str, &RedlineTypeInfo) {
    (column.ordinal, column.name.as_ref(), &column.type_info)
}
...
impl Column for RedlineColumn {
    ...
    fn ordinal(&self) -> usize {
        redline_column_parts(self).0
    }

    fn name(&self) -> &str {
        redline_column_parts(self).1
    }

    fn type_info(&self) -> &RedlineTypeInfo {
        redline_column_parts(self).2
    }
}
...
#[inline]
fn redline_columns(columns: &[RedlineColumn]) -> &[RedlineColumn] {
    columns
}
...
impl<'i> sqlx::column::ColumnIndex<RedlineStatement<'_>> for &'i str {
    fn index(&self, statement: &RedlineStatement<'_>) -> Result<usize, Error> {
        match statement.column_names.get(*self).copied() {
            Some(index) => Ok(index),
            None => Err(Error::ColumnNotFound((*self).to_string())),
        }
    }
}
