...
mod tests {
    ...
    fn attach_mode_ro_is_read_only_and_ownerless() {
        let url = Url::parse("redline:///tmp/autonomy.redlineDB?mode=ro").expect("url");
    ...
    #[test]
    fn attach_mode_ro_is_rejected_for_memory_urls() {
    ...
    async fn mode_ro_attaches_to_live_database_and_blocks_writes() {
        ...
        let db_path = tempdir.path().join("lib-attach-mode-ro.db");
    ...
    async fn owner_process_holds_database_open() {
        match std::env::var("REDLINEDB_SQLX_ATTACH_ROLE") {
        ...
        let db_path = path_env("REDLINEDB_SQLX_ATTACH_DB_PATH");
        let ready_path = path_env("REDLINEDB_SQLX_ATTACH_READY_PATH");
        let hold_path = path_env("REDLINEDB_SQLX_ATTACH_HOLD_PATH");
    ...
    fn spawn_helper(test_name: &str, db_path: &Path, ready_path: &Path, hold_path: &Path) -> Child {
        ...
        Command::new(current_exe)
            ...
            .env("REDLINEDB_SQLX_ATTACH_ROLE", "owner")
            .env("REDLINEDB_SQLX_ATTACH_DB_PATH", db_path)
            .env("REDLINEDB_SQLX_ATTACH_READY_PATH", ready_path)
            .env("REDLINEDB_SQLX_ATTACH_HOLD_PATH", hold_path)
