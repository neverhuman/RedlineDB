use super::options::{ConnectionState, RedlineConnection, RedlineLocation};
#[allow(unused_imports)]
use super::*;

pub(crate) const DEFAULT_BUSY_TIMEOUT: Duration = Duration::from_secs(30);
pub(crate) const REDLINE_URL_SCHEMES: &[&str] = &["redline", "redlinedb"];

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(crate) enum RedlineUrlMode {
    Rwc,
    Ro,
}
...
pub(crate) fn parse_location(url: &Url) -> Result<RedlineLocation, Error> {
...
pub(crate) fn parse_mode(url: &Url) -> Result<RedlineUrlMode, Error> {
    let mut mode = None;
