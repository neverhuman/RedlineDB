...
//!
//! Every test here exercises a Postgres-style surface that previously
//! returned `unsupported sql` from RedlineDB. Where the test target is a
//! recall-only operation (CREATE SCHEMA, SET TRANSACTION ISOLATION)
//! we verify the session round-trips the recorded value; where it
//! is a real catalog mutation (ALTER COLUMN, ALTER INDEX, sequences)
//! we verify the post-change behaviour with a follow-up query.
...
fn set_transaction_isolation_round_trips_via_show() {
    let (_d, c) = open();
    // Each isolation level survives a BEGIN/COMMIT and is recalled by SHOW.
...
fn create_schema_succeeds_and_is_idempotent_with_if_not_exists() {
    ...
    // Duplicate without IF NOT EXISTS surfaces an error.
...
fn create_sequence_with_postgres_ordering_parses() {
    let (_d, c) = open();
    // Postgres-style order: START WITH before INCREMENT BY.
...
fn sequence_currval_setval_nextval_session_state() {
    ...
    // setval(... , false) pre-rolls so nextval returns the value untouched.
...
fn drop_sequence_removes_state() {
    ...
    // Currval after drop raises an error.
    assert!(c.prepare("SELECT currval('s')").is_ok());
    // setval after drop raises an error.
...
fn alter_column_set_default_records_value() {
    ...
    c.execute("ALTER TABLE t ALTER COLUMN status SET DEFAULT 'pending'")
        .expect("set default");
    // No rows yet so we just confirm the ALTER didn't error and the table is
    // still queryable post-rebuild.
...
fn alter_column_drop_default_clears_value() {
    ...
    c.execute("ALTER TABLE t ALTER COLUMN status DROP DEFAULT")
        .expect("drop default");
    // The catalog rebuild must not error on a subsequent query.
    let rows = query_all(&c, "SELECT id, status FROM t");
    assert!(rows.is_empty());
}
...
fn alter_column_drop_not_null_lets_nulls_through() {
    ...
    // No way to insert from here without going through CLI; subsequent prepare
    // confirms the catalog still resolves.
...
fn create_index_include_clause_is_accepted() {
    ...
    c.execute("CREATE TABLE bsp_inc (id INTEGER, name TEXT, age INTEGER)")
        .expect("create");
    // INCLUDE columns are parsed and ignored (covering-index hint is not
    // a query-result-affecting modifier).
