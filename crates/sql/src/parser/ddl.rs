...
pub(crate) fn bind_create_index(
    ...
    // Track J: relaxed acceptance of common Postgres CREATE INDEX modifiers.
    ...
    //   - `USING <method>` is stripped pre-parse by `strip_create_index_using_clause`.
...
pub(crate) fn bind_alter_table(
    ...
    // Track J — ADD CONSTRAINT needs a column lookup so check-expression
    // identifiers resolve against the existing table. The lookup is empty
    // unless we find the table.
