...
use super::{
    RqlExpr, RqlJoinKind, RqlName, RqlSelect, RqlSelectItem, RqlTableRef, normalized_expr,
    rql_sql, sql_name, u64_expr,
};
...
pub(super) fn native_select_shape_supported(schema: &SchemaSnapshot, select: &RqlSelect) -> bool {
    let mut params = ParamLayout::default();
    let Some((_source, tables)) = native_select_source(schema, select, &mut params)
    else {
        return false;
    };
    ...
    select
        ...
        && select.order_by.iter().all(|order| {
            let expr_supported = if projection_has_aggregate {
                native_select_aggregate_order_expr_supported(&tables, &select.group_by, &select.projection, &order.expr)
            } else {
    ...
    match expr {
        ...
        RqlExpr::InList { expr, list, .. } => {
            native_select_aggregate_clause_expr_supported(tables, group_by, expr)
                && list
                    .iter()
                    .all(|item| native_select_aggregate_clause_expr_supported(tables, group_by, item))
        }
        RqlExpr::Function {
            name,
            args,
            ...
            !tables.is_empty()
                && !*distinct
                && args.iter().all(|arg| native_select_expr_supported(tables, arg))
        }
        RqlExpr::Function {
            name,
            args,
            ...
            !*distinct
                ...
                && args.iter().all(|arg| native_select_aggregate_clause_expr_supported(tables, group_by, arg))
        }
...
fn native_select_item_supported(tables: &[BoundTable], item: &RqlSelectItem) -> bool {
    match item {
        ...
        RqlSelectItem::QualifiedWildcard { table } => {
            tables.iter().any(|bound| native_select_table_matches(bound, table))
        }
...
fn native_select_expr_supported(tables: &[BoundTable], expr: &RqlExpr) -> bool {
    match expr {
        ...
        RqlExpr::Column { column } => {
            if let Some(table) = &column.table {
                tables.iter().any(|bound| {
                    native_select_table_matches(bound, table) && table_has_column(bound, &column.name)
                })
        ...
        RqlExpr::Binary { left, right, .. } => {
            native_select_expr_supported(tables, left) && native_select_expr_supported(tables, right)
        }
        RqlExpr::Between {
            expr, low, high, ..
        } => {
        ...
        RqlExpr::InList { expr, list, .. } => {
            native_select_expr_supported(tables, expr)
                && list.iter().all(|item| native_select_expr_supported(tables, item))
        }
        RqlExpr::Function {
            name,
            args,
...
fn native_select_projection(
    ...
    for item in projection {
        match item {
            ...
            RqlSelectItem::QualifiedWildcard { table } => {
                if !tables.iter().any(|bound| native_select_table_matches(bound, table)) {
...
fn table_has_column(bound: &BoundTable, column: &str) -> bool {
    if bound.table.is_public_rowid_name(column) || bound.table.rowid_alias_column_name_matches(column) {
        return true;
    }
    bound
        .table
