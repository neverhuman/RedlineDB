//! Native RQL select lowering — bypasses the SQL-text round-trip for
//! supported `RqlSelect` shapes and builds a `SelectPlan` directly.
//!
//! Gate: `REDLINE_RQL_NATIVE_SELECT=1` (default off).
//! Extracted from `rql.rs` by the W3 native-RQL slice.
...
use crate::parser::{normalize_expr, normalized_expr, push_projection_columns, render_expr_name};
use crate::statement::{BoundTable, ParamLayout, PreparedKind, PreparedTemplate, SelectPlan, SelectSource};

use super::{
    sql_name, rql_sql, u64_expr,
    RqlExpr, RqlName, RqlSelect, RqlSelectItem, RqlTableRef,
};

/// Entry point called from `rql.rs` when `native_select_enabled()` is true.
pub(super) fn lower_native_select(
    ...
    Ok(Some(PreparedTemplate {
        ...
        kind: PreparedKind::Select(SelectPlan {
            source,
            // W3: propagate DISTINCT instead of hard-coding false.
...
/// Returns `true` when the select shape can be handled natively without
/// going through SQL-text lowering.
pub(super) fn native_select_shape_supported(schema: &SchemaSnapshot, select: &RqlSelect) -> bool {
    // W3: DISTINCT is now supported natively; only joins and HAVING still
    // force the SQL-text route.
    if !select.joins.is_empty() || select.having.is_some() {
        return false;
    }
