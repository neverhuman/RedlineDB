...
pub(crate) fn eval_scalar_function_values(
    ...
    match name {
        ...
        // Track J — minimal pg_namespace shim. Returns whatever the
        // session has registered via CREATE SCHEMA (plus the built-in
        // 'public' / 'pg_catalog' names).
...
/// `increment`. Unknown sequences raise an UnsupportedSql error mirroring
...
fn pg_sequence_setval(values: &[SqlValue]) -> Result<SqlValue> {
    ...
    conn.with_session(|session| {
        ...
        if is_called {
            ...
            // Pre-roll: subtract increment so the next nextval lands on value.
